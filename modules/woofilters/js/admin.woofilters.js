...
"use strict";
	...
	AdminPage.prototype.wpfAddFilter = (function(id, uniqId, settings, options) {
		var _this = this,
			_noOptionsFilters = this.$noOptionsFilters,
			wpfGetPreviewInit = false,
			template = jQuery('.wpfOptionsTemplate .wpfFilterOptions[data-filter="'+id+'"]'),
			skipAttributeTermsLoad = options && options.skipAttributeTermsLoad;
		...
		if( id === 'wpfAttribute' ){
			...
			text = text + ' - ' + title;
			if (!skipAttributeTermsLoad && blockTemplate.find('select[name="f_list"]').val() != '0') {
